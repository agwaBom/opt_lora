...
def tokenize_input(examples):
    ...
    # model_inputs outputs dict {"input_ids", "attention_mask"}
    model_inputs = tokenizer(inputs, max_length=512, add_special_tokens=True)
    return model_inputs

def tokenize_label(examples):
    labels = ["Translate this from German to English: \nGerman: " + example['de'] + "\nEnglish:" + example['en'] for example in examples["translation"]]
    # model_inputs outputs dict {"input_ids", "attention_mask"}
    model_labels = tokenizer(labels, max_length=512, add_special_tokens=True)
    return model_labels
...
if __name__ == "__main__":
    ...
    with open('./'+opt.model.replace("/", "_")+'_pred_list.txt', mode='w') as out:
